	...
	render(){
			if(this.props.chatText){
				return this.props.chatText.map((item,i) => {
					...
					return <SpeechBubbleMe key={`textID-${item.textID}`} address={item.address}  id={`textID-${item.textID}`} ipAddress={item.ipAddress} chatItem={item.message}  elementHeight={elementHeight} datetime={item.datetime}/>
					})
			}else{
				return null;
			}
